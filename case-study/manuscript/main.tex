...
occurred during follow-up. Median follow-up was approximately
26 months. AJCC pathologic stage I/II accounted for the majority of
patients with non-missing stage; stage III/IV represented a smaller
fraction.

\textbf{Primary outcome.}
The apparent C-index of AJCC pathologic stage alone was 0.60; the
apparent C-index of the additive model AJCC + \HCCRTS was 0.72 (full
numbers in Table~1 and \texttt{tcga\_bootstrap\_metrics.json}). The
apparent $\Delta$C-index was approximately 0.12. After Harrell-Lee-Mark
optimism correction over 1000 bootstrap iterations, the corrected
$\Delta$C-index was approximately \emph{[INSERT FROM JSON]} with 95\%
bootstrap CI \emph{[lo, hi]}. The primary hypothesis $\Delta C \geq 0.02$
with CI lower bound > 0 is \emph{[INSERT FROM JSON]}.

\textbf{Secondary outcomes.}
L1: 4-strata log-rank across HCC-TRS quartiles in TCGA-LIHC was highly
significant ($\chi^2 \approx 69$, $p < 10^{-14}$; Figure~1). L3:
time-dependent AUCs at 1, 3, 5 years were $\approx 0.80, 0.77, 0.79$.
L4: calibration slope was $\approx 0.76$, indicating mild over-
fitting in the apparent fit (expected; the optimism-corrected
estimate is the primary outcome). L5: Schoenfeld global $p$ for
\HCCRTS was \emph{[INSERT FROM JSON]}, $\geq 0.05$ indicating the
proportional-hazards assumption is not violated for the score
covariate at the global level. L7: the permutation null distribution
for $\Delta$C-index had mean $\approx 0$, with permutation $p$ \emph{[INSERT
FROM JSON]}.

\textbf{External cohort (GSE10143).}
After probe-to-gene mapping and intersect-restricted rebuilding of
\HCCRTS (per prereg-v2 Section B, triggered when signature coverage
in the external cohort fell below 80\%), the TCGA-locked \HCCRTS was
applied to the 80 GSE10143 HCC tumour samples (32 OS events). The
C-index in GSE10143 was \emph{[INSERT FROM JSON]} with 95\% bootstrap
CI \emph{[lo, hi]}; the median-split log-rank $p$ was \emph{[INSERT
FROM JSON]}.

\textbf{Robustness.}
Stage-adjusted Cox (\texttt{robustness\_metrics.json}) shows that
\HCCRTS remains an independent prognostic factor for OS after
adjustment for AJCC stage (HR for one-SD increase in \HCCRTS is
\emph{[INSERT FROM JSON]}, $p$ \emph{[INSERT FROM JSON]}). In a 5-
fold cross-validation, the mean C-index of the combo model
(\texttt{cv\_metrics.json}) was \emph{[INSERT FROM JSON]}.
