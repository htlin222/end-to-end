...
In May 2026 a Google DeepMind team published in \textit{Nature} the Co-Scientist multi-agent system\citep{coscientist2026}, demonstrating that an LLM-driven workflow can autonomously generate, critique and refine biomedical research hypotheses, with three of its proposed acute-myeloid-leukaemia drug-repurposing candidates validated \textit{in vitro}. Two months earlier, the Sakana AI team's end-to-end automation pipeline\citep{luSakana2026} produced a manuscript that passed first-round workshop peer review at a top-tier machine-learning conference, the first AI-authored paper to do so under blind review. \textit{Nature}'s accompanying editorial called explicitly for institutions, funders and publishers to respond\citep{natureEditorial2026}. The 2026 question is no longer whether autonomous agentic research is feasible; it is what the published unit should look like when one occurs.
...
\textbf{Scientific hallucination.} Fabricated citations, invented effect sizes and plausible-but-incorrect claims appear when an LLM produces a literature review or a methods section under loose supervision. The Sakana group's authors enumerated this failure mode explicitly in their Nature paper's Limitations section\citep{luSakana2026}. Current Lancet policy responds by prohibiting the activity. This is a category error: prohibiting the activity does not prevent its undeclared occurrence; it incentivises camouflage. Disclosure 2.0 makes the activity visible, which is the precondition for catching the hallucinations the current policy fears.
...
\textbf{``AI is your copilot, not the pilot.''} A vigorous and well-implemented school argues that the right response to LLM scientific reasoning is to embed it in a human-supervised pipeline with explicit integrity gates. The Academic Research Skills toolkit\citep{ars2026}, a Claude Code plugin with over 15\,000 GitHub stars by May 2026, operationalises this position with a ten-stage orchestrator, claim-level citation audits, and an opt-in calibration mode that measures its own false-negative and false-positive rates. We do not refute this position; we propose that ``operator-sovereign autonomy'' and ``copilot-supervised'' are two legitimate equilibria with different cost-of-error profiles, and that Disclosure 2.0 is the disclosure layer that both should share. The current Lancet policy serves neither equilibrium well.
...
\textbf{Permit reviewer subagents on the operator's side, subject to the same disclosure rules.} The proposed manifest captures author-side reviewer subagents in the same machine-readable structure that a future editor-side pilot would use; symmetric disclosure is a necessary condition for any future editor-side reviewer-AI deployment.
